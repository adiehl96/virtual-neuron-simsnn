...
def run(x, y, steps=100, plotting=False, options=None):
    options = {} if options is None else options
    net = Network()
    sim = Simulator(net)

    # Precision (we can only have integer precision,
    # as fractional precision doesn't work in this calculation)
    precision = [5, 4, 4, 4]  # 16-bit integer precision
    ...
    # X and Y for this iteration
    a_pos = dec_to_bin_list(x, precision[0], precision[1])
    a_neg = int_to_bin_list(0, negative_precision)
    b_pos = dec_to_bin_list(y, precision[0], precision[1])
    b_neg = int_to_bin_list(0, negative_precision)
    ...
    BG = demux_vn_to_bg(Y, BG)
    IG, read_out = pass_bg_to_ig(net, precision, BG)

    # dangerzone, dict to list doesn't guarantee order todo: enforce order
    # sim.raster.addTarget(list(X.z_positive.values()))
    # sim.raster.addTarget(X.z_positive[4])
    # sim.raster.addTarget(BG[0][4])
    # sim.multimeter.addTarget(X.z_positive[4])
    # sim.multimeter.addTarget(BG[0][4])

    BG = pass_and_shift_vn_to_bg(X)
    BG = demux_vn_to_bg(Y, BG)

    # sim.raster.addTarget(X.z_positive[4])
    # sim.raster.addTarget(Y.z_positive[4])
    # sim.raster.addTarget(BG[4])
    # sim.multimeter.addTarget(X.z_positive[4])
    # sim.multimeter.addTarget(BG[4])
    ...
    sim.run(steps, plotting=plotting, options=options)
    rasterdata = sim.raster.get_measurements()
    print("rasterdata.any()", rasterdata.any())
...
def create_bg(net, n, precision):
    ...
    bg = [
        [
            net.createLIF(ID=f"bg_neuron_{j}_{i}", thr=2)
            for i in range(precision[0] + precision[1])
        ]
        for j in range(n)
    ]
    return bg
...
def pass_vn_to_bg(X, BG=None):
    ...
    """
    net = X.net
    BG = create_bg(net, X.positive_precision, X.precision) if BG is None else BG
    for i in range(X.positive_precision):
        for j in range(X.positive_precision):
            net.createSynapse(
                pre=X.z_positive[j],
                post=BG[i][j],
...
def pass_and_shift_vn_to_bg(X, BG=None):
    ...
    net = X.net
    BG = create_bg(net, X.positive_precision, X.precision) if BG is None else BG
    for bg_idx in range(X.positive_precision):
        z_pos_idxs = list(
            range(np.max([X.precision[1] - bg_idx, 0]), X.positive_precision)
        )
        BG_intern_idxs = list(
            range(np.max([-X.precision[1] + bg_idx, 0]), X.positive_precision)
        )
        BG_intern_idxs = BG_intern_idxs[: len(z_pos_idxs)]

        for z_pos_idx, BG_intern_idx in zip(z_pos_idxs, BG_intern_idxs):
            net.createSynapse(
                pre=X.z_positive[z_pos_idx],
                post=BG[bg_idx][BG_intern_idx],
                ID=f"{X.z_positive[z_pos_idx].ID}_{BG[bg_idx][BG_intern_idx].ID}",
                w=1,
                d=1,
            )
    return BG
...
def pass_bg_to_ig(net, precision, BG, IG=None):
    ...
    ig_size = int(np.ceil(int("1" * (precision[0] + precision[1]), 2) / 2))
    IG = create_ig(net, ig_size, precision) if IG is None else IG
    for i in range(len(BG)):
        for j in range((precision[0] + precision[1])):
            if (
                not j % 2
