...
for t in range(num_runs):
	...
	for i in range(B.positive_precision):
		nest.Connect(B.z_positive[i], C.y_positive[i], syn_spec={"weight": 0.0, "delay": 1.0})

	for i in range(B.negative_precision):
		nest.Connect(B.z_negative[i], C.y_negative[i], syn_spec={"weight": 0.0, "delay": 1.0})
	...
	if (c_pos_dec == a_pos_dec) and (c_neg_dec == a_neg_dec):
		print("\nCorrect Results!")
	else:
		print("\nIncorrect Results!")
		break
